 ...
 */
export async function dedupe(clientId: string | null, text: string, hash: string, selfMessageId: string | null = null): Promise<DedupeOutcome> {
  if (!clientId) return { kind: "new" };
  const cfg = noise();
  const days = cfg.duplicate_window_days;

  // A message never matches itself: a re-run whose earlier run already made a card must not be filed as a repeat of
  // its own card (that turned a 14-item meeting into "14 on existing cards" on 2026-09-15).
  const exact = await sql()`
    ...
    where m.client_id = ${clientId} and m.text_hash = ${hash}
      and (${selfMessageId}::uuid is null or m.id <> ${selfMessageId}::uuid)
