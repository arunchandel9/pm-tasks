  ...
  return createMcpHandler(
    (server) => {
      ...
      server.registerTool("search_tasks", {
        title: "Search tasks",
        description: "Tasks by client, status, department or words in the title/ask. Every card on the Pulp department boards is known (origin 'hub' = made by the hub, 'sheet' = a row in the PM sheet, 'board' = read from the board itself), each with its board, list (status), labels, assignee and due date. A board card whose client the hub could not tell (no client label, no client name in the title) has client null: to find those, search by words or department without a client. status: open (default), done, overdue, waiting (on client), staging (not yet approved), all. days limits to tasks created in the last N days; from/to to tasks created on those calendar days.",
        inputSchema: z.object({
      ...
      server.registerTool("hub_status", {
        ...
        const polls = await sql()`select key, value from settings where key in ('pulp_poll_last','gmail_poll_last','gchat_last_event','slack_last_event','voice_last','sheet_cards_last','board_mirror_last','meet_poll_last','tick_last','chat_inbox_last')`;
