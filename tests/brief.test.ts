...
describe("daily brief", () => {
  it("is one quiet line when there is nothing to act on", () => {
    ...
    expect(b.headline).toContain("quiet day");
    expect(b.detail).toBeNull();
  });
  it("counts in the headline, only non-empty sections in the thread, links on cards", () => {
    const b = renderBrief({
      ...empty,
    ...
    expect(b.headline).toBe("📋 *Daily brief · Mon 14 Sep* · 1 new · 2 waiting on you · 1 overdue · 1 done · details in the thread");
