...
- **Ask the hub from Claude:** "what is pending for HOH", "what happened in the last meeting with TED", "what happened on 12 September", "anything overdue this week". To create a card from Claude, it asks who is asking, the client, the ask and who to assign it to; the card is made at once (To Do, assigned, sheet row) and the feed shows the outcome, nothing to tap there. The card reads "Anuj via Claude-Arun". Every card on the boards is known to Claude, sheet row or not; put the client's short name in the title or a client label on a card you make by hand, so Claude files it under that client.
