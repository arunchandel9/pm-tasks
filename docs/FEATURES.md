...
| 1.6 | Voice notes, short | Audio forwarded to the DM (WhatsApp notes included, with or without a file extension) goes to Google Speech-to-Text v2 with the Chirp model, which decodes the file itself (no sample-rate guessing) and handles accents; falls back to v2 "long", then to the v1 engine. Indian English. Then processed like text. If every engine returns nothing, the DM says so and asks for the text. | Built (engine changed 2026-09-11) |
