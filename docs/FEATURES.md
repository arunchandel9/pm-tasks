...
| 2.10 | Understood, no card | An update, question, idea or decision (routing types with `no_card`) makes no card but is never silent: one ℹ️ "noted, no card" line in the feed with the words in its thread, and the sender's thread gets "Nothing created: read as an update…" (2026-09-15, after a strategy voice note vanished without a trace). | Built |
| 2.9 | Unhappy client, no ask | When a client's message has no ask but reads as displeased ("Too little too late"), the hub makes no card and posts one ⚠️ line in the feed: client, the words, the source link, "a person should reply". The message is listed in the brief under "Waiting on you" until handled. Added 2026-09-15 after a forwarded client reply was misread as a task. | Built |
