...
| 1.10 | MCP `add_request` | A PM's Claude or Codex files a request straight into the hub; it lands in Staging like any other. Claude accounts are shared, so the connector never tells Claude who the owner is, and the tool requires both the name the person gave and a flag that Claude asked for it in the chat (tightened 2026-09-15 after Claude filled in the owner's name unasked); feed and sheet read "Claude, Priya (PM Claude A)": person, then the account in brackets, or just the name when they are the same. One key per Claude account, named after it. The only write action exposed; approving, moving or closing cards from an assistant is not possible by design (2026-09-14). | Built |
