...
| 1.10 | MCP `add_request` | A PM's Claude or Codex files a request straight into the hub; it lands in Staging like any other. Claude accounts are shared, so the tool requires the name of the person asking (Claude is told to ask, never guess); feed and sheet read "Assistant, Priya (PM Claude A)": person, then the account in brackets. One key per Claude account, named after it. The only write action exposed; approving, moving or closing cards from an assistant is not possible by design (2026-09-14). | Built |
