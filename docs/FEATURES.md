...
| 1.6 | Voice notes, short | Audio forwarded to the DM (WhatsApp notes included, with or without a file extension) is transcribed with Google Speech-to-Text, Indian English first, the Opus sample rate read from the file, a second pass at the other rate when the first comes back with almost nothing. Then processed like text. | Built (tuned 2026-09-11) |
