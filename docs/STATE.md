...
- **2026-09-23, the hub reads the whole boards (Arun):** a PM asked Claude for the HBOT videos on the Video Sprint board
  ...
  Four boards are past the 1000 cap (SEO, Writers, Development, Video). Tried live and not accepted by Pulp:
  `?offset=`, `?page=`, `GET /lists/{id}/cards`; the probes were removed. Pulp is MangoEyes' own tool, so the fix is
  there (a higher cap or a paging parameter, open with Arun); until then a capped board is read as the 1000 Pulp
  returns, which includes the recently made and moved cards, and nothing is archived on it. Closed cards are dropped.
  Labels name clients loosely ("House of Health (HoH)", "HOH - House Of Health", "TED (The Eye Doctor)"): matched by
  whole-word name or alias inside the label; a card mirrored without a client gets one on the next pass.
